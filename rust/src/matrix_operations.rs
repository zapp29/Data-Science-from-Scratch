pub fn shape(m: Vec<Vec<f32>>) -> (i32, i32) {
...
pub fn get_row(m: Vec<Vec<f32>>, r: usize) -> Vec<f32> {
    m[r].clone()
}

pub fn get_column(m: Vec<Vec<f32>>, c: usize) -> Vec<f32> {
...
mod tests {
    ...
    #[test]
    fn test_shape() {
        let m1 = vec![vec![1.0, 2.0, 3.0], vec![1.0, 2.0, 3.0]];
        assert_eq!(shape(m1), (2, 3));

        let m2 = vec![vec![1.0, 2.0]];
    ...
    #[test]
    fn test_get_row() {
        let m = vec![vec![1.0, 2.0, 3.0], vec![1.0, 2.0, 3.0]];
    ...
    fn test_get_column() {
        ...
        assert_eq!(get_column(m.clone(), 0), result1);
        assert_eq!(get_column(m.clone(), 1), result2);
        assert_eq!(get_column(m.clone(), 2), result3);
    }
